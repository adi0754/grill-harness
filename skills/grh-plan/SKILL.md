...
找到内核后运行 `python3 <主内核>/scripts/grh.py entry-check --entry grh-plan --project <项目绝对路径>`，用户范围更窄时追加 `--requested-scope`。资格检查通过后只按主内核契约执行允许范围。

等待最终规格批准时，先用 30 秒结论说明所选路线、仓库挑战结论、关键未验证假设和验证边界，再明确告诉用户正在回答“这份实施合同是否足以授权写代码”。不得只展示 `final_spec_approval`、产物版本或批准命令而不解释授权含义。
