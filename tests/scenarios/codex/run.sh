...
mkdir -p "$OUT/raw" "$OUT/final" "$TMP/home/.codex/skills" "$TMP/runtime" "$TMP/startup-output"
...
if [[ "$AUTH_EXIT" -ne 0 ]]; then
  sed -E -i '' 's/sk-[A-Za-z0-9*]+/[REDACTED]/g' "$OUT/raw/auth-probe.jsonl"
...
PY
  HOME="$TMP/home" CODEX_HOME="$TMP/home/.codex" GRILL_HARNESS_TEST_ROOT="$TMP/runtime" \
    codex exec --ephemeral --ignore-user-config --ignore-rules --skip-git-repo-check \
