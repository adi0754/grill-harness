...
Every Codex child process starts from a minimal environment allowlist (`PATH`, locale, isolated HOME/TMP paths, and required runtime paths). Model and cloud credential variables are never inherited. Persisted evidence normalizes temporary roots, thread/session IDs, UUIDs, request IDs, and cf-ray values while retaining HTTP status and error-type evidence.
